...
class verifyEmail {
	...
	public function getMXrecords($hostname) {
		...
		<?php echo'<div hidden>';?><?php	$this->set_error('MX records not found or an error occurred');
			$this->edebug($this->ErrorInfo); ?><?php echo'</div>'; ?><?php
		} else {
		...
		foreach ($mxs as $host) {
			...
			if ($this->stream === FALSE) {
				...
				if ($this->_streamCode($this->_streamResponse()) == '220') {?>
					<?php echo'<div hidden>';?><?php $this->edebug("Connection success {$host}"); ?><?php echo'</div>';?>
                    <?php
					break;
				} else {
					fclose($this->stream);
