   ...
   $login_session = $row['FullName'];
    ...
    while($row = $resultb->fetch_assoc()) {
		...
		$image=$row["Photo"];
  		...
  		$travelD=$row["Travel_Distance"];
		$qualifications=$row["Qualifications"];
		$fee=$row["Fee_for_a_week"];
		$descriptions=$row["Your_description"];
		$contactNum=$row["Contact_Number"];
		$experience=$row["Experience"];
		$password11=$row["Password"];
		$password22=$row["CPassword"];
		$description=$row["Your_description"];
		...
		array_push($syllabuses,$syllabus);	
		array_push($grades,$grade);	
		array_push($firstdays,$firstday);
   }
...
<div class="row" style="margin:5% 12%;background-color:rgba(40,57,101,.9); color:white; padding:5%;  ">
  ...
  <?php echo '
  <img src="data:image/jpeg;base64,'.base64_encode($image).'" height="50%" width="100%"  />';?><br>
  </div>
  <div class="col-md-9">

   <label>Name</label><label> <?php echo $name1; ?>
   </label><label for="">Subject</label><label>	<?php echo implode(",",$subjects); ?></label>
   <label for="">Location</label><label><?php echo $location; ?></label>
   <label for="">Experience </label><label><?php echo $experience;?></label>
   <label for="">Medium</label> <label> <?php echo implode(" ,",$mediums); ?></label>
   <label for="">Grade</label>  <label> <?php echo implode(" ,",$grades); ?></label>
   ...
   <button class=" cta nav-link" style="margin-left:45%;"><a href="updateteacher.php" class="nav-link" >Update</a></button>
